        ...
        public function save(&$errors = array()) {
            if($this->validate($errors)) {
                ...
                try {
					...
					$sql = "REPLACE INTO file (name, type, kind, size) VALUES(:name, :type, :kind, :size)";
					$values = array(':name'=>$this->name, ':type'=>$this->type, ':kind'=>$this->kind, ':size'=>$this->size);
		...
		*/
		...
		public function validate(&$errors = array()) {
			...
			if(is_uploaded_file($this->tmp)) {
				...
				if(!empty($this->size)) {
					$max_upload_size = 2 * 1024 * 1024 * 1024; // = 2097152 (2 megabytes)
	    ...
	    static public function get ($id) {
            try {
                $query = static::query("
                    ...
                    WHERE id = :id
                    ", array(':id' => $id));
                $file = $query->fetchObject(__CLASS__);
                return $file;
            } catch(\PDOException $e) {
                return false;
            }
        ...
        public static function getAll ($id, $which) {
            ...
            try {
                $sql = "SELECT file FROM {$which}_file WHERE {$which} = ? ORDER BY file ASC";
                $query = self::query($sql, array($id));
                foreach ($query->fetchAll(\PDO::FETCH_ASSOC) as $file) {
                    $files[] = self::get($file['file']);
                }
		...
		public static function getAdfiles ($id) {
            ...
            try {
                $sql = "SELECT id FROM file WHERE kind = 3 ORDER BY id ASC";
                $query = self::query($sql, array($id));
                foreach ($query->fetchAll(\PDO::FETCH_ASSOC) as $ad) {
                    $adfile[] = self::get($ad['id']);
                }
        ...
        public function remove($which) {
            ...
            try {
                ...
                if (\is_string($which) /*&& \in_array($which, self::$types)*/) {
                    $sql = "DELETE FROM {$which}_file WHERE file = ?";
                    $query = self::query($sql, array($this->id));
                }
