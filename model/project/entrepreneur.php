    ...
    class Entrepreneur extends \Goteo\Core\Model {
		...
		public static function getAll ($project, $lang = null) {
            try {
				...
				$query = self::query("SELECT * FROM entrepreneur WHERE project = :project", array(':project' => $project));
				//era static::
				
                //$sql = "SELECT * FROM entrepreneur WHERE project = :project";
						/*support.id as id,
		...
		public function validate(&$errors = array()) {
            ...
            if (empty($this->project))
                $errors[] = 'Não tem projeto pra colocar o cara!';
...
*/
		...
		public function save (&$errors = array()) {
			...
			try {
				$sql = "REPLACE INTO entrepreneur SET " . $set;
				self::query($sql, $values);
    			if (empty($this->id)) $this->id = self::insertId();
				return true;
			} catch(\PDOException $e) {
				$errors[] = "A pessoa {$data['name']} no se ha grabado correctamente. Por favor, revise los datos." . $e->getMessage();
                return false;
			}
		}
		...
		public function remove (&$errors = array()) {
            ...
            try {
                self::query("DELETE FROM entrepreneur WHERE id = :id AND project = :project", $values);
                ...
                $errors[] = 'Nao deu para apagar o empreendedor ' . $this->project . ' ' . $e->getMessage();
